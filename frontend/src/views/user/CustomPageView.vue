...
// SECURITY: Validate iframe src URLs to only allow https: and relative paths
// This prevents token/data exfiltration via http: or javascript: iframe src
DOMPurify.addHook('uponSanitizeElement', (node, data) => {
  if (data.tagName === 'iframe' && node instanceof Element) {
    const src = node.getAttribute('src')
    if (src && !src.startsWith('https://') && !src.startsWith('/') && !src.startsWith('./') && !src.startsWith('#')) {
      node.remove()
    }
  }
})
...
async function fetchAndRenderMarkdown(slug: string) {
  ...
  try {
    ...
    const sanitized = DOMPurify.sanitize(html, {
      ADD_TAGS: ['iframe'],
      ADD_ATTR: ['allowfullscreen', 'frameborder', 'src', 'sandbox'],

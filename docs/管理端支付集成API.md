...
### 目标
本文档用于对接外部支付系统（如 `zsyqpay`）与 智算引擎 的 Admin API，覆盖：
- 支付成功后充值
- 用户查询
- 人工余额修正
- 前端购买页参数透传
...
```json
{
  ...
  "value": 100.0,
  "user_id": 123,
  "notes": "zsyqpay order: cm1234567890"
}
```
...
当 智算引擎 打开 `purchase_subscription_url` 或用户侧自定义页面 iframe URL 时，会统一追加：
- `user_id`
- `token`
- `theme`（`light` / `dark`）
...
### 6) `doc_url` 配置建议
- 查看链接：`https://github.com/nameyzh-netizen/zsyq/blob/main/docs/管理端支付集成API.md`
- 下载链接：`https://raw.githubusercontent.com/nameyzh-netizen/zsyq/main/docs/管理端支付集成API.md`
...
### Purpose
This document describes the minimal 智算引擎 Admin API surface for external payment integrations (for example, `zsyqpay`), including:
...
When 智算引擎 opens `purchase_subscription_url` or a user-facing custom page iframe URL, it appends:
- `user_id`
- `token`
- `theme` (`light` / `dark`)
...
- View URL: `https://github.com/nameyzh-netizen/zsyq/blob/main/docs/管理端支付集成API.md`
- Download URL: `https://raw.githubusercontent.com/nameyzh-netizen/zsyq/main/docs/管理端支付集成API.md`

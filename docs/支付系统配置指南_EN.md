...
智算引擎 has a built-in payment system that enables user self-service top-up without deploying a separate payment service.
...
- [Migrating from 智算引擎Pay](#migrating-from-zsyqpay)
...
## Migrating from 智算引擎Pay

If you previously used [智算引擎Pay](https://github.com/touwaeriol/zsyqpay) as an external payment system, you can migrate to the built-in payment system:

### Key Differences

| Aspect | 智算引擎Pay | Built-in Payment |
|--------|-----------|-----------------|
| Deployment | Separate service (Next.js + PostgreSQL) | Built into 智算引擎, no extra deployment |
| Payment Methods | EasyPay, Alipay, WeChat, Stripe | Same |
| Configuration | Environment variables + separate admin UI | Unified in 智算引擎 admin dashboard |
| Top-up Integration | Via Admin API callback | Internal processing, more reliable |
| Subscription Plans | Supported | Not yet (planned) |
| Order Management | Separate admin interface | Integrated in 智算引擎 admin dashboard |

### Migration Steps

1. Enable payment in 智算引擎 admin dashboard and configure providers (use the same payment credentials)
2. Update webhook callback URLs to 智算引擎's callback endpoints
3. Verify that new orders are processed correctly via built-in payment
4. Decommission the 智算引擎Pay service

> **Note**: Historical order data from 智算引擎Pay will not be automatically migrated. Keep 智算引擎Pay running for a while to access historical records.

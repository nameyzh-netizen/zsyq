...
> **智算引擎 公式ドメイン：`zsyq.cloud`、`zsyq.shop`。「智算引擎」の名称を使用している他のウェブサイトはすべてサードパーティによるデプロイであり、本プロジェクトとは無関係です。**
...
- **マルチアカウント集約管理** - OAuth、APIキーなど多種の上流アカウントを統合管理。バッチインポートと自動ローテーション対応
- **APIキー配布と制御** - ユーザー向け APIキーのワンクリック生成。クォータ割当、権限分離、使用量追跡をサポート
- **精密な課金とコスト計算** - トークンレベルのリアルタイム使用量追跡、多モデル差別化価格、倍率・割引戦略対応
- **スマートスケジューリングと負荷分散** - スティッキーセッション + アカウントローテーション + 自動障害除外で高可用性を保障
- **同時実行とレートの二重制御** - ユーザー/アカウント単位の同時実行制限 + リクエスト/トークン二次元レート制限で滥用・過負荷を防止
- **内蔵決済とセルフサービスチャージ** - 外部依存ゼロ。EasyPay、Alipay、WeChat Pay、Stripe を統合し、即時セルフサービスチャージ（[設定ガイド](docs/支付系统配置指南_EN.md)）
- **管理ダッシュボードとリアルタイム監視** - Web画面で一元運用：使用量ダッシュボード、アカウント状態、ログ監査とアラート
- **外部システム連携** - iframe でチケット・通知などの外部システムを埋め込み、管理機能をシームレスに拡張

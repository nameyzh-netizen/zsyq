...
# 或：
#   sudo ./install-datamanagementd.sh --source /path/to/zsyq/repo

BIN_PATH=""
SOURCE_PATH=""
INSTALL_DIR="/opt/zsyq"
DATA_DIR="/var/lib/zsyq/datamanagement"
SERVICE_FILE_NAME="zsyq-datamanagementd.service"
...
参数:
  ...
  --source  指定 zsyq 仓库路径（脚本会执行 go build）
  -h, --help 显示帮助

示例:
  sudo ./install-datamanagementd.sh --binary ./datamanagement/datamanagementd
  sudo ./install-datamanagementd.sh --source /opt/zsyq-src
EOF
}
...
if ! id zsyq >/dev/null 2>&1; then
  echo "[2/6] 创建系统用户 zsyq..."
  useradd --system --no-create-home --shell /usr/sbin/nologin zsyq
else
  echo "[2/6] 系统用户 zsyq 已存在，跳过创建"
fi
...
echo "[4/6] 准备数据目录..."
mkdir -p "$DATA_DIR"
chown -R zsyq:zsyq /var/lib/zsyq
chmod 0750 "$DATA_DIR"
...
systemctl enable --now zsyq-datamanagementd

echo "[6/6] 完成，当前状态："
systemctl --no-pager --full status zsyq-datamanagementd || true

cat <<'EOF'

下一步建议：
1. 查看日志：sudo journalctl -u zsyq-datamanagementd -f
2. 在 zsyq（容器部署时）挂载 socket:

...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="智算引擎 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 智算引擎 服务："
    ...
    ["not_installed"]="智算引擎 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 智算引擎。"
    ...
    ["server_config_desc"]="配置 智算引擎 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="智算引擎 Installation Script"
    ...
    # Completion
    ["install_complete"]="智算引擎 installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start 智算引擎 service:"
    ...
    ["not_installed"]="智算引擎 is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove 智算引擎 from your system."
    ...
    ["uninstall_complete"]="智算引擎 has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install 智算引擎"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove 智算引擎"
    ...
    ["server_config_desc"]="Configure 智算引擎 server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/zsyq" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/zsyq" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="zsyq_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    local expected_checksum=$(awk -v name="$archive_name" '$2 == name {print $1; exit}' "$TEMP_DIR/checksums.txt")
    ...
    # Copy binary
    cp "$TEMP_DIR/zsyq" "$INSTALL_DIR/zsyq"
    chmod +x "$INSTALL_DIR/zsyq"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/zsyq"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/zsyq.service << EOF
[Unit]
Description=智算引擎 - 智能算力与 AI 服务网关
Documentation=https://github.com/nameyzh-netizen/zsyq
...
User=zsyq
Group=zsyq
WorkingDirectory=/opt/zsyq
ExecStart=/opt/zsyq/zsyq
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=zsyq
...
enable_autostart() {
    ...
    if systemctl enable zsyq 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status zsyq"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u zsyq -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart zsyq"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop zsyq"
    echo ""
    echo "=============================================="
}
...
upgrade() {
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/zsyq" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet zsyq; then
        print_info "$(msg 'stopping_service')"
        systemctl stop zsyq
    fi

    # Backup current binary
    cp "$INSTALL_DIR/zsyq" "$INSTALL_DIR/zsyq.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/zsyq.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/zsyq"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start zsyq

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: 智算引擎 must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/zsyq" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="zsyq.backup.${current_version}"
        else
            backup_name="zsyq.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/zsyq" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop zsyq 2>/dev/null || true
    systemctl disable zsyq 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/zsyq.service
    systemctl daemon-reload

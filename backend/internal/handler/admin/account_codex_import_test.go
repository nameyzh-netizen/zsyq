...
func TestNormalizeCodexImportExpiredJWTWithoutStructuredExpiryFallsBackToWarning(t *testing.T) {
	expiredToken := buildCodexImportTestJWT(t, time.Now().Add(-time.Hour), map[string]any{})

	item, err := normalizeCodexImportEntry(codexImportEntry{Index: 1, Value: expiredToken})
	if err != nil {
		t.Fatalf("normalizeCodexImportEntry error = %v, want nil", err)
	}
	if item == nil {
		t.Fatal("normalizeCodexImportEntry item = nil")
	}
	if len(item.WarningTexts) == 0 {
		t.Fatal("warning_texts should not be empty for JWT without structured expiry fallback")
	}
}

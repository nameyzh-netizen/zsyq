...
import (
	...
	"github.com/nameyzh-netizen/zsyq/internal/pkg/openai"
...
func decodeCodexJWTClaims(token string) (*codexJWTClaims, error) {
	...
	// Without signature verification, anyone can forge claims (sub, email, org_id, etc.)
	// which are used for account identity matching and credential enrichment.
	// We use jwt.Parse with a key function that rejects unverified tokens.
	// Since we don't have the issuer's public key, we validate structural integrity
	// and expiry, but explicitly warn that claims come from an untrusted source.
	// The access_token itself must be functional for the account to be useful.
	...
	// SECURITY: Validate expiry to reject stale/forged tokens with expired timestamps.
	// Clock skew tolerance of 120 seconds allows minor time differences.
	now := time.Now()
	if claims.Exp != 0 && now.Unix()-120 > claims.Exp {
		return nil, fmt.Errorf("codex JWT claims expired (exp=%d, now=%d)", claims.Exp, now.Unix())
	}
	// SECURITY: Validate issuer - reject tokens without a recognized issuer.
	// Known valid issuers for Codex/OpenAI tokens.
	if claims.Iss != "" {
		...
		if !knownIssuers[claims.Iss] {
			slog.Warn("codex JWT claims have unrecognized issuer; treating as unverified",
				"issuer", claims.Iss)
		}
	}
	slog.Warn("codex JWT claims decoded without signature verification; claims are from an untrusted source and must not be used for authorization decisions",

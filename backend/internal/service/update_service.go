...
func (s *UpdateService) PerformUpdate(ctx context.Context) error {
	...
	for _, asset := range info.ReleaseInfo.Assets {
		if strings.Contains(asset.Name, archiveName) && !strings.HasSuffix(asset.Name, ".txt") {
			if downloadURL != "" {
				return fmt.Errorf("multiple compatible releases found for %s/%s", runtime.GOOS, runtime.GOARCH)
			}
	...
	if checksumURL == "" {
		return fmt.Errorf("checksums.txt is required for secure updates")
	}
	...
	tempDir, err := os.MkdirTemp(exeDir, ".zsyq-update-*")
	if err != nil {
	...
	// Verify checksum before extracting or replacing the binary.
	if err := s.verifyChecksum(ctx, archivePath, checksumURL); err != nil {
		return fmt.Errorf("checksum verification failed: %w", err)
	}

	// Extract binary from archive
	newBinaryPath := filepath.Join(tempDir, "zsyq")
...
func (s *UpdateService) verifyChecksum(ctx context.Context, filePath, checksumURL string) error {
	...
	if len(checksumData) > maxChecksumSize {
		return fmt.Errorf("checksums file too large: %d bytes", len(checksumData))
	}
...
func (s *UpdateService) extractBinary(archivePath, destPath string) error {
	...
	if strings.Contains(archivePath, ".tar") {
		...
		for {
			...
			if baseName == "zsyq" || baseName == "zsyq.exe" {
...
func extractBinaryFromZip(archivePath, destPath string) error {
	...
	for _, file := range zr.File {
		...
		written, copyErr := io.Copy(out, io.LimitReader(rc, maxBinarySize+1))
		closeErr := rc.Close()
		if copyErr != nil {
			_ = out.Close()
			return copyErr

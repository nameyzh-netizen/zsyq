...
import (
	...
	"github.com/nameyzh-netizen/zsyq/internal/pkg/antigravity"
...
func (s *SettingService) GetPublicSettings(ctx context.Context) (*PublicSettings, error) {
	...
	return &PublicSettings{
		...
		SiteName:                         s.getStringOrDefault(settings, SettingKeySiteName, "智算引擎"),
		SiteLogo:                         settings[SettingKeySiteLogo],
		SiteSubtitle:                     s.getStringOrDefault(settings, SettingKeySiteSubtitle, "智能算力与 AI 服务网关"),
...
const adminAPIKeyHashAlgorithm = "sha256"
...
func (s *SettingService) GenerateAdminAPIKey(ctx context.Context) (string, error) {
	...
	encoded, err := encodeAdminAPIKeyRecord(key)
	if err != nil {
		return "", err
	}

	if err := s.settingRepo.Set(ctx, SettingKeyAdminAPIKey, encoded); err != nil {
...
func (s *SettingService) GetAdminAPIKeyStatus(ctx context.Context) (maskedKey string, exists bool, err error) {
	value, err := s.settingRepo.GetValue(ctx, SettingKeyAdminAPIKey)
	if err != nil {
		if errors.Is(err, ErrSettingNotFound) {
			return "", false, nil
		}
...
// ValidateAdminAPIKey 验证管理员 API Key，旧明文存储会在成功验证后迁移为 hash 记录。
func (s *SettingService) ValidateAdminAPIKey(ctx context.Context, key string) (bool, error) {
	...
	if record, ok := parseAdminAPIKeyRecord(stored); ok {
		if record.Algorithm != adminAPIKeyHashAlgorithm || record.Hash == "" {
			return false, nil
		}
		computed := hashAdminAPIKey(key)
		return subtle.ConstantTimeCompare([]byte(computed), []byte(record.Hash)) == 1, nil
	}
	...
	if err := s.settingRepo.Set(ctx, SettingKeyAdminAPIKey, encoded); err != nil {
		return false, fmt.Errorf("migrate admin api key: %w", err)
	}
	return true, nil
}
...
func encodeAdminAPIKeyRecord(key string) (string, error) {
	record := adminAPIKeyRecord{
		Version:     1,
		Algorithm:   adminAPIKeyHashAlgorithm,
		Hash:        hashAdminAPIKey(key),
		Fingerprint: fingerprintAdminAPIKey(key),
		CreatedAt:   time.Now().UTC().Format(time.RFC3339),
	}
	data, err := json.Marshal(record)
	if err != nil {
		return "", fmt.Errorf("encode admin api key record: %w", err)
	}
	return string(data), nil
}

func parseAdminAPIKeyRecord(value string) (adminAPIKeyRecord, bool) {
...
func fingerprintAdminAPIKey(key string) string {
	fingerprint := hashAdminAPIKey(key)

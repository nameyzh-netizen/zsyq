...
func TestInjectSiteTitle(t *testing.T) {
	t.Run("replaces_title_with_site_name", func(t *testing.T) {
		html := []byte(`<html><head><title>智算引擎 - 智能算力与 AI 服务网关</title></head><body></body></html>`)
		settingsJSON := []byte(`{"site_name":"MyCustomSite"}`)

		result := injectSiteTitle(html, settingsJSON)

		assert.Contains(t, string(result), "<title>MyCustomSite - 智能算力与 AI 服务网关</title>")
		assert.NotContains(t, string(result), "智算引擎")
	})
	...
	t.Run("returns_unchanged_when_title_has_attributes", func(t *testing.T) {
		...
		html := []byte(`<html><head><title lang="en">智算引擎</title></head><body></body></html>`)
	...
	t.Run("preserves_rest_of_html", func(t *testing.T) {
		html := []byte(`<html><head><meta charset="UTF-8"><title>智算引擎</title><script src="app.js"></script></head><body><div id="app"></div></body></html>`)
		...
		assert.Contains(t, string(result), "<title>TestSite - 智能算力与 AI 服务网关</title>")
	})
}

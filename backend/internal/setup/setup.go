...
type AdminConfig struct {
	...
	AllowGeneratedPassword bool   `json:"-" yaml:"-"`
}
...
func setupPostgresDSN(cfg *DatabaseConfig, dbName string) string {
	parts := []string{
...
func TestDatabaseConnection(cfg *DatabaseConfig) error {
	...
	defaultDSN := setupPostgresDSN(cfg, "postgres")
	...
	if !exists {
		_, err := db.ExecContext(ctx, fmt.Sprintf("CREATE DATABASE %s", quotePostgresIdentifier(cfg.DBName)))
		if err != nil {
	...
	targetDSN := setupPostgresDSN(cfg, cfg.DBName)
...
func createAdminUser(cfg *SetupConfig) (bool, string, error) {
	...
	if strings.TrimSpace(cfg.Admin.Password) == "" {
		if !cfg.Admin.AllowGeneratedPassword {
...
func AutoSetupEnabled() bool {
	return envTruthy("AUTO_SETUP")
}

func autoSetupAllowGeneratedAdminPassword() bool {
	return envTruthy("AUTO_SETUP_ALLOW_GENERATED_ADMIN_PASSWORD")
}
...
func AutoSetupFromEnv() error {
	...
	cfg := &SetupConfig{
		Database: DatabaseConfig{
			...
			DBName:   getEnvOrDefault("DATABASE_DBNAME", "zsyq"),
		...
		Admin: AdminConfig{
			Email:                  getEnvOrDefault("ADMIN_EMAIL", "admin@zsyq.local"),
			Password:               getEnvOrDefault("ADMIN_PASSWORD", ""),
			AllowGeneratedPassword: autoSetupAllowGeneratedAdminPassword(),
		},
		Server: ServerConfig{
	...
	if strings.TrimSpace(cfg.Admin.Password) == "" && !cfg.Admin.AllowGeneratedPassword {
		return fmt.Errorf("ADMIN_PASSWORD is required when AUTO_SETUP is enabled; set AUTO_SETUP_ALLOW_GENERATED_ADMIN_PASSWORD=true only for development or ephemeral deployments")
	}

...
import (
	...
	ippkg "github.com/nameyzh-netizen/zsyq/internal/pkg/ip"
	"github.com/nameyzh-netizen/zsyq/internal/pkg/response"
	"github.com/nameyzh-netizen/zsyq/internal/pkg/sysutil"
...
type RouteOptions struct {
	...
	AllowRemoteInit bool
}

// NewSetupToken generates a one-time setup token and the hash stored by setup routes.
func NewSetupToken() (string, string, error) {
	bytes := make([]byte, 32)
...
func RegisterRoutes(r *gin.Engine, opts ...RouteOptions) {
	...
	setup := r.Group("/setup")
	{
		...
		protected.Use(setupGuard(routeOpts))
		{
...
func setupGuard(opts RouteOptions) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !opts.AllowRemoteInit && !isLocalSetupRequest(c) {
			response.Error(c, http.StatusForbidden, "Setup changes are only allowed from localhost")
			c.Abort()
			return
		}
		if !validateSetupToken(c.GetHeader("X-Setup-Token"), opts.SetupTokenHash) {
			response.Error(c, http.StatusUnauthorized, "Invalid setup token")
			c.Abort()
			return
		}
		c.Next()
...
func validateSetupToken(token, expectedHash string) bool {
	token = strings.TrimSpace(token)
	expectedHash = strings.TrimSpace(expectedHash)
	if token == "" || expectedHash == "" {
		return false
	}
	actualHash := hashSetupToken(token)
...
func isLocalSetupRequest(c *gin.Context) bool {
	...
	return ippkg.IsLoopbackIP(host)
}

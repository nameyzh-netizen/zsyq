...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	if allowMissingJWTSecret && originalJWTSecret == "" {
		// Generate a random temporary secret for setup mode instead of a predictable placeholder.
		// This prevents token forgery if the setup endpoint is accidentally exposed.
		tmpSecret, err := generateJWTSecret(32)
		if err != nil {
			return nil, fmt.Errorf("generate temporary JWT secret for setup: %w", err)
		}
		cfg.JWT.Secret = tmpSecret
		slog.Warn("Using randomly generated temporary JWT secret for setup mode. This will be replaced after setup completes.")
	}
	...
	if cfg.JWT.Secret != "" && isWeakJWTSecret(cfg.JWT.Secret) {
		return nil, fmt.Errorf("jwt.secret is weak/insecure; use a 32+ character random secret in production (current value matches a known weak secret)")
	}
...
func setDefaults() {
	...
	viper.SetDefault("log.service_name", "zsyq")
	...
	viper.SetDefault("database.dbname", "zsyq")
	...
	// Pricing - 默认使用本地 fallback 文件；如需启用远程同步，请在 config.yaml 中显式配置 remote_url/hash_url
	...
	viper.SetDefault("dashboard_cache.key_prefix", "zsyq:")

...
func TestLoadForBootstrapAllowsMissingJWTSecret(t *testing.T) {
	...
	if len(cfg.JWT.Secret) < 32 {
		t.Fatalf("LoadForBootstrap() should supply a validation-safe bootstrap JWT secret, got %q", cfg.JWT.Secret)
	}
}
...
func TestLoadDefaultDashboardCacheConfig(t *testing.T) {
	...
	if cfg.Dashboard.KeyPrefix != "zsyq:" {
		t.Fatalf("Dashboard.KeyPrefix = %q, want %q", cfg.Dashboard.KeyPrefix, "zsyq:")
	}

...
| 数据库凭据 | user=`zsyq`, password=`zsyq`, dbname=`zsyq` |
...
psql -U zsyq -h 127.0.0.1 -d zsyq -f temp.sql
```
...
3. 无密码登录并重置
   ...
   ALTER USER zsyq WITH PASSWORD 'zsyq';
...
```bash
# 连接数据库
psql -U zsyq -h 127.0.0.1 -d zsyq
...
# 执行 SQL 文件
psql -U zsyq -h 127.0.0.1 -d zsyq -f migration.sql
```
...
- [上游仓库](https://github.com/nameyzh-netizen/zsyq)

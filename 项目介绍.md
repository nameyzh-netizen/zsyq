...
- **多账号聚合管理** - 统一管理 OAuth、API Key 等多种上游账号，支持批量导入与自动轮换
- **API Key 分发与管控** - 为用户一键生成 API Key，支持配额分配、权限隔离与用量追踪
- **精确计费与成本核算** - Token 级别实时用量追踪，多模型差异化定价，支持倍率与折扣策略
- **智能调度与负载均衡** - 粘性会话 + 账号轮换 + 故障自动剔除，保障请求高可用
- **并发与速率双重管控** - 用户级、账号级并发限制 + 请求/Token 双维度速率限制，防滥用与过载
- **内置支付与自助充值** - 零外部依赖，集成 EasyPay 易支付、支付宝、微信、Stripe，用户自助充值即时到账（[配置指南](docs/支付系统配置指南.md)）
- **管理后台与实时监控** - Web 界面一站式运维：用量看板、账号状态、日志审计与告警
- **外部系统集成** - iframe 嵌入工单、通知等外部系统，无缝扩展管理后台能力
...
- 克隆/更新源码到 `/opt/zsyq`
- 生成 `POSTGRES_PASSWORD`、`JWT_SECRET`、`TOTP_ENCRYPTION_KEY`、`ADMIN_PASSWORD`
